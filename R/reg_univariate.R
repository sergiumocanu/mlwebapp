...
reg_univariate <- function (args) {  
  ...
  if (sig.method == "fdr" &&
      length(which(fit_dfm$adj.P.Val < UNI_ALPHA)) == 0 &&
      length(which(fit_dfm$P.Value < UNI_ALPHA)) == 0) {
    NO_SIG_WARNING <- TRUE
  } else if (sig.method == "fdr" &&
             length(which(fit_dfm$adj.P.Val < UNI_ALPHA)) == 0) {
    FDR_FAIL_WARNING <- TRUE
    sig.method <- "none"
  } else if (sig.method == "none" &&
             length(which(fit_dfm$P.Value < UNI_ALPHA)) == 0) {
    NO_SIG_WARNING <- TRUE
  }
  if (UNI_FDR) {
